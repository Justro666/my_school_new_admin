...
const form = useForm({
    ...
    category_name: props.categoriesInfo.name,
    category_description: props.categoriesInfo.description
});

let showError = ref(false);

const submit = () => {
    Inertia.put(route("categoryTool.update", form.id), form, {
        onError: (data) => {
...
<template>
    ...
    <div class="absolute h-auto w-full overflow-x-hidden py-5 headercustomleft top-32 customblack transition-all duration-500 pr-36" :class="showMenu ? 'ml-60 pl-4 pr-64' : 'ml-28'">
        ...
        <div class="py-5">
            <button>
                <a href="/categoryTool" class="underline underline-offset-4 hidden md:block text-whiteTextColor">back</a>
            </button>
        </div>
    </div>
</template>
