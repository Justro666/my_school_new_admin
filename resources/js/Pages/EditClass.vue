...
let imageFile = ref(props.classdata[0].c_profile);
let input = null
...
<Header headername="Web Developer Batch 9" classaddmode=true class="w-full pr-36 transition-all duration-500" :class="showMenu ? 'ml-60 pl-4 pr-64' : 'ml-28'"/>
<loading v-model:active="isloading" :is-full-page="fullPage" :loader="loader" :color="color" :width="width"
        :height="height" />

<!---------------- body ----------------------->
<div class="absolute w-full headercustomleft  top-32 customblack px-5 transition-all duration-500 pr-36" :class="showMenu ? 'ml-60 pl-4 pr-64' : 'ml-28'">
...
<div class="my-5">
   ...
   <img :src="imageFile" alt="" class="rounded-full sm:w-20 sm:h-20 h-14 w-14" >
...
<input id="file-upload" type="file" accept="image/*" @change="onFile" @input="form.classimage = $event.target.files[0]"/>
