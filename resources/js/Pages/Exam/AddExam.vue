...
const submit = () => {
    if (form.failmark >= form.fullmark) {
        ...
        Inertia.post(route("exam.store"), form, {
            onSuccess: (data) => {
...
<template>
    ...
    <Header headername="Add Exam" class="w-full pr-36 transition-all duration-500" :class="showMenu ? 'ml-60 pl-4 pr-64' : 'ml-28'"/>
    <div
        class="absolute h-auto w-full headercustomleft top-32 bg-elementBackground rounded-lg mt-5 ml-5 px-5 py-8 transition-all duration-500 pr-36" :class="showMenu ? 'ml-64 pl-4 pr-64' : 'ml-28'"
    >
        <form @submit.prevent="submit">
            ...
            <Link
                href=""
                class="flex item-center py-2 px-5 mx-8 text-whiteTextColor text-md bg-blueTextColor rounded-xl float-right mt-5"
                type="button"
            >
                <span class="font-semibold">Mark Entry</span>
            </Link>
        </form>
    </div>
</template>
